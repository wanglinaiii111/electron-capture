...
const { BrowserWindow, ipcMain, globalShortcut,systemPreferences } = require('electron');
const os = require('os');
...
const captureScreen = (e, args) => {
    if (os.platform() === "darwin") {
        ...
        let permissions = systemPreferences.getMediaAccessStatus("screen");
    ...
    captureWins = displays.map((display) => {
        ...
        captureWin.on('show', () => {
            console.log('____')
            setTimeout(()=>{
                captureWin.webContents.send('capture-screenText', { type: captureType });
            },100)
        });
